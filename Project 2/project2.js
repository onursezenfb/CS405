...
class MeshDrawer {
	...
	constructor() {
        ...
        this.normalLoc = gl.getAttribLocation(this.prog, 'normal');
		 ...
		 // Default light position and colors
		this.lightPos = [0.0, 0.0, 1.0]; // Example light position
	...
	draw(trans) {
		...
		// Set the normal attribute for the lighting calculations
        gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer);
        gl.enableVertexAttribArray(this.normalLoc);
        gl.vertexAttribPointer(this.normalLoc, 3, gl.FLOAT, false, 0, 0);

        // Set the light position and colors
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			// Use mipmaps for power of 2 sized textures
			gl.generateMipmap(gl.TEXTURE_2D);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.REPEAT);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_LINEAR);
		} else {
			// Settings for non power of 2 sized textures
...
const meshFS = `
			...
			void main() {
				...
				if (enableLighting) {
					...
					vec3 ambient = ambientIntensity * baseColor.rgb; // Calculate ambient light
